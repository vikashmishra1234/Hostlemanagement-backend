...
const markattendece4 = async(req,res)=>{
   let today = new Date();
   ...
   try {
      const exist = await fourthYear.findOne({Mobile:req.body.Mobile,createdAt:{$gte:`${tarik}`}})
      if(exist){
         return res.json({success:false,error:"Marked already"});
      }
    const newStudent = await fourthYear(req.body) ;
